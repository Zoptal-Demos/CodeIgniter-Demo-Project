...
class User_model extends CI_Model{
 ...
 /**************************Add Hotel section ****************************/
	...
	public function get_hotel($id){
		$this->db->select("*");
		$this->db->from("hotels");
		$where = "(id = '".$id."' AND status = 0 AND archive = 1)";
		$this->db->where($where);
		$result = $this->db->get();
		//echo $this->db->last_query(); die;
		$res = $result->row_array();
...
/**************set user geofence**************/
	...
	public function add_location_test($arr, $id){
		$arr["status"] = 0;
		$arr["archive"]  = 1;
		$arr["user_id"]  = $id;
 		$arr["time"]    = time();
		return $this->db->insert("test_location_update", $arr);
	}
	...
	public function hotels_location_api($lat,$long){
		$where="status= 1";
		$sql  = "select * FROM (SELECT *, (3959 * acos (cos(radians('".$lat."'))* cos(radians( latitude))* cos(radians(longitude) - radians('".$long."') )+ sin(radians('".$lat."'))* sin(radians(latitude)))) AS distances FROM (hotels)) AS t  where distances < 2 AND ".$where;
		$result = $this->db->query($sql);
		return $result->result_array();
